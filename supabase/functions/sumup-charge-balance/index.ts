...
import { corsHeaders, getBusinessSumupCredentials, penceToMajor, sumupFetch } from '../_shared/sumup.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const credentials = await getBusinessSumupCredentials(supabaseAdmin, booking.business_id as string)
    if (!credentials) return json({ error: 'This business has not connected a payment provider yet.' }, 400)
    ...
    const checkoutRes = await sumupFetch(credentials.apiKey, '/v0.1/checkouts', {
      method: 'POST',
      body: JSON.stringify({
    ...
    const processRes = await sumupFetch(credentials.apiKey, `/v0.1/checkouts/${checkout.id}`, {
      method: 'PUT',
      body: JSON.stringify({
        payment_type: 'card',
