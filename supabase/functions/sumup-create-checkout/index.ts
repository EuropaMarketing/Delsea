...
import { corsHeaders, ensureSumupCustomer, getBusinessSumupCredentials, penceToMajor, sumupFetch } from '../_shared/sumup.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: booking, error: bErr } = await supabase
      .from('bookings')
      .select(`
        id, business_id, spots_booked, discount_amount, gift_voucher_amount,
    ...
    await ensureSumupCustomer(credentials.apiKey, customer)
