-- ============================================================
-- Multi-spot booking support for group sessions
-- Run this in Supabase SQL Editor.
-- ============================================================

ALTER TABLE bookings ADD COLUMN IF NOT EXISTS spots_booked INTEGER NOT NULL DEFAULT 1;

-- Updated create_booking: capacity check uses SUM(spots_booked)
...
BEGIN
  ...
  -- Capacity check uses SUM so multi-spot bookings are counted correctly
  IF v_is_group THEN
  ...
  INSERT INTO bookings (
    ...
    starts_at, ends_at, notes, status, spots_booked
  )
  VALUES (
    p_business_id, v_customer_id, v_staff_id, p_service_id, p_variant_id,
    p_starts_at, p_ends_at, p_notes, 'confirmed', p_spots_booked
  )
